...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user.isAcceptingMessages) {
      return NextResponse.json({ success: false, message: `This user ${username} is not accepting messages` }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: false, message: error instanceof Error ? error.message : "Failed to send Message" }, { status: 500 });
  }
}
